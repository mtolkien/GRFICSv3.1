...
def merge_files(directory_path, num_rows_per_category, chunk_size=10000):
    ...
    categories_found = set()  # Per tracciare le categorie trovate
    total_attack_rows = 0
    ...
    # Rimuovi la categoria 'Benign' dai file di attacchi, se presente
    ...
    # Processa idle.csv solo per le righe benigne
    ...
    if os.path.exists(idle_file):
        ...
        for chunk in pd.read_csv(idle_file, chunksize=chunk_size):
            idle_filtered_rows = pd.concat([idle_filtered_rows, chunk], ignore_index=True)
            total_idle_rows += len(chunk)

            if total_idle_rows >= idle_rows_needed:
                break

        final_idle_filtered = idle_filtered_rows.sample(n=min(total_idle_rows, idle_rows_needed), random_state=1)
    ...
    print(f'\nNumero totale di righe maligne: {total_attack_rows}')
