...
def process_category(file_path, category, num_rows_per_category, exclude_df, chunk_size=10000):
    ...
    for chunk in pd.read_csv(file_path, chunksize=chunk_size):
        ...
        if not category_filtered.empty:
            # Filter to exclude rows already present in the exclusion DataFrame
        ...
        # If you have reached or exceeded the required number of rows, stop
...
def merge_files(directory_path, num_rows_per_category, benign_file_path, exclude_file_path, output_file,
    ...
    # Read the exclusion CSV into a DataFrame
    exclude_df = pd.read_csv(exclude_file_path)

    categories_found = set()

    # Find all categories present in the CSV files
    ...
    print(f"Attack categories found: {categories_found}")
    ...
    # Show the row count for each category
    category_counts = final_data['Type of connection'].value_counts()
    for category, count in category_counts.items():
        print(f'Number of rows for the category "{category}": {count}')
